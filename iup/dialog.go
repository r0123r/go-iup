...
func LayoutDialog(h IHandle) IHandle {
...
func Image(width, height int, pixels []byte) IHandle {
	return toHandle(C.IupImage(C.int(width), C.int(height), (*C.uchar)(&pixels[0])))
}

func ImageRGB(width, height int, pixels []byte) IHandle {
	return toHandle(C.IupImageRGB(C.int(width), C.int(height), (*C.uchar)(&pixels[0])))
}
